...
class MySidelineDataService {
  ...
  async findExistingMySidelineCarnival(eventData) {
    ...
    if (eventData.mySidelineId) {
      ...
      if (match) {
        console.log(
          `Found existing MySideline carnival by ID: ${eventData.mySidelineId} - "${eventData.title}"`
        );
        return match;
      }
    }
    ...
    if (eventData.mySidelineTitle) {
      ...
      if (eventData.mySidelineDate !== undefined) {
        ...
        if (
          ...
          console.warn(
            `Invalid mySidelineDate for carnival "${eventData.mySidelineTitle}": ${eventData.mySidelineDate}`
          );
        }
      }
      ...
      if (match) {
        console.log(
          `Found existing MySideline carnival by legacy fields: "${eventData.mySidelineTitle}"`
        );
        return match;
      }
    }
    ...
    if (eventData.date && eventData.title) {
      ...
      if (isValidDate) {
        ...
        if (match) {
          console.log(
            `Found existing MySideline carnival by date and title: "${eventData.title}" on ${eventData.date}`
          );
          return match;
        }
      } else {
        console.warn(
          `Invalid date for carnival "${eventData.title}": ${eventData.date}. Skipping date-based matching.`
        );
      }
    }
      ...
      try {
        ...
        if (existingCarnival) {
          if (eventData.isActive === false && existingCarnival.isActive === false) {
            ...
            console.log(
              `Skipping update of inactive carnival: ${eventData.mySidelineTitle} on ${eventData.date} at ${eventData.locationAddress}`
            );
          ...
          if (Object.keys(updateData).length > 2) {
            // > 2 because id and lastMySidelineSync are always included
            await existingCarnival.update(updateData);
            console.log(
              `Updated ${Object.keys(updateData).length - 2} empty fields for carnival: ${
                eventData.title
              }`
            );
          } else {
      ...
      if (shouldSync) {
        ...
        if (!lastSync) {
          ...
          const hoursSinceLastSync =
            (new Date() - new Date(lastSync.completedAt)) / (1000 * 60 * 60);
          console.log(
            `Running MySideline sync (last sync was ${hoursSinceLastSync.toFixed(1)} hours ago)...`
          );
        }
        return true;
      } else {
        ...
        console.log(
          `MySideline sync skipped - recent sync found (${hoursSinceLastSync.toFixed(1)} hours ago)`
        );
        return false;
      }
    } catch (error) {
  ...
  parseDate(dateString) {
    ...
    const patternMonthName =
      /^(?:(\d{1,2})\s+([a-zA-Z]{3,})\s+(\d{4}))|(?:([a-zA-Z]{3,})\s+(\d{1,2}),?\s+(\d{4}))$/i;
  ...
  async deactivatePastCarnivals() {
    ...
    try {
      ...
      pastCarnivals.forEach((carnival, index) => {
        ...
        console.log(
          `   ${index + 1}. "${carnival.title}" (${carnival.state}) - ${daysPast} days past`
        );
      });
  ...
  /**
   * Deactivate carnivals that have a MySideline ID but are missing from the current scrape
   * This ensures our database doesn't keep showing carnivals that have been removed from MySideline
   * EXCEPTION: Future dated carnivals are NOT deactivated even if missing from scrape
   *
   * @param {Array} scrapedCarnivals - Array of scraped carnival objects
   * @returns {Promise<Object>} Result object with count of deactivated carnivals
   */
  async deactivateMissingCarnivals(scrapedCarnivals) {
    console.log('️🕵️ Checking for missing MySideline carnivals...');

    try {
      // 1. Extract MySideline IDs from the scraped data
      // Filter out any entries that somehow don't have an ID
      const currentMySidelineIds = scrapedCarnivals.map((c) => c.mySidelineId).filter((id) => id); // Filter out null/undefined/empty strings

      if (currentMySidelineIds.length === 0) {
        console.log(
          '⚠️ No MySideline IDs found in scraped data. Skipping deactivation safety check.'
        );
        return { success: true, deactivatedCount: 0, message: 'No IDs in scraped data' };
      }

      // 2. Find all active carnivals in our DB that match:
      // - Have a mySidelineId (so we know they came from the source)
      // - Are currently active
      // - Are NOT in the list of IDs we just scraped
      const carnivalsToCheck = await Carnival.findAll({
        where: {
          mySidelineId: {
            [Op.not]: null,
            [Op.ne]: '',
          ...
          isActive: true,
          // NOT IN the list of IDs we just scraped
          mySidelineId: {
            [Op.notIn]: currentMySidelineIds,
          },
        },
        attributes: ['id', 'title', 'date', 'mySidelineId', 'state'],
      });

      if (carnivalsToCheck.length === 0) {
        console.log('✅ No missing carnivals found.');
        return { success: true, deactivatedCount: 0 };
      }

      console.log(
        `📋 Found ${carnivalsToCheck.length} carnivals missing from scrape. Checking dates...`
      );

      // 3. Filter using the "Future Date Exception"
      // We only deactivate if the carnival is in the past OR has no date
      const today = new Date();
      today.setHours(0, 0, 0, 0);

      const carnivalsToDeactivate = carnivalsToCheck.filter((carnival) => {
        // If no date, safe to clean up as it's likely bad data
        if (!carnival.date) return true;

        // If date is in the future, KEEP IT ACTIVE (User Exception)
        if (new Date(carnival.date) >= today) {
          console.log(`   Keeping future carnival active: "${carnival.title}" (${carnival.date})`);
          return false;
        }

        // Otherwise (past date), deactivate it
        return true;
      });

      if (carnivalsToDeactivate.length === 0) {
        console.log('✅ All missing carnivals are future-dated. No deactivations needed.');
        return { success: true, deactivatedCount: 0 };
      }

      console.log(`🗑️ Deactivating ${carnivalsToDeactivate.length} stale carnivals...`);
      carnivalsToDeactivate.forEach((c) => {
        console.log(`   - "${c.title}" (ID: ${c.mySidelineId})`);
      });

      // 4. Perform the update
      const idsToDeactivate = carnivalsToDeactivate.map((c) => c.id);
      const [updatedCount] = await Carnival.update(
        {
          isActive: false,
          updatedAt: new Date(),
        },
        {
          where: {
            id: {
              [Op.in]: idsToDeactivate,
            },
          },
        }
      );

      console.log(`✅ Successfully deactivated ${updatedCount} missing carnivals`);
      ...
      console.error('❌ Error deactivating missing carnivals:', error.message);
      return {
        success: false,
        error: error.message,
        deactivatedCount: 0,
