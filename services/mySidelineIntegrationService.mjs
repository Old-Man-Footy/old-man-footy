...
class MySidelineIntegrationService {
  ...
  async checkAndRunInitialSync() {
    try {
      ...
      console.log(
        'Initial MySideline sync will be skipped. You can manually trigger it from the admin panel.'
      );
    }
  }
  ...
  async syncMySidelineCarnivals() {
    ...
    try {
      ...
      const cleanedCarnivals = scrapedCarnivals
        .map((carnival) => {
          try {
            return this.scraperService.validateAndCleanData(carnival);
          } catch (validationError) {
            console.warn(
              `Failed to validate carnival "${carnival.title}": ${validationError.message}`
            );
      ...
      console.log(
        `${cleanedCarnivals.length}/${scrapedCarnivals.length} carnivals passed validation`
      );
      ...
      // Step 2.5: Cleanup missing carnivals (that are NOT in the future)
      await this.dataService.deactivateMissingCarnivals(cleanedCarnivals);

      // Count new vs updated carnivals for logging
      const carnivalsCreated = processedCarnivals.filter(
        (carnival) =>
      ...
      const imageDownloadPromises = processedCarnivals
        .filter((carnival) => carnival.clubLogoURL && carnival.clubLogoURL.startsWith('http'))
        .map((carnival) => {
      ...
      if (results && results.length > 0) {
        ...
        results.forEach(async (result) => {
          ...
          if (result.success) {
            console.log(
              `Logo downloaded successfully for carnival ${result.entityId}: ${result.publicUrl}`
            );
            if (carnival) {
              try {
                ...
                console.error(
                  `❌ Failed to update carnival ${carnival.id} logo URL:`,
                  updateError.message
                );
              }
            } else {
            ...
            console.warn(
              `Failed to download logo for carnival ${result.entityId}: ${result.error}`
            );
            if (carnival) {
              try {
                await Carnival.update({ clubLogoURL: null }, { where: { id: carnival.id } });
      ...
      console.log(
        `MySideline sync completed. Processed ${processedCarnivals.length} carnivals (${carnivalsCreated} new, ${carnivalsUpdated} updated).`
      );
  ...
  async fetchCarnivals() {
    try {
      ...
      if (result && result.success) {
        console.log(
          `Manual MySideline sync completed successfully. Found ${result.carnivalsProcessed} carnivals.`
        );
