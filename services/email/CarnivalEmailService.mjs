...
export class CarnivalEmailService extends BaseEmailService {
  ...
  async sendCarnivalNotification(carnival, type = 'new') {
    try {
      ...
      if (!this._canSendEmails()) {
        this._logBlockedEmail(
          'Carnival Notification',
          'subscribers',
          `${type} carnival: ${carnival.title}`
        );
        return {
          success: false,
      ...
      const promises = subscriptions.map(async (subscription) => {
        const unsubscribeUrl = `${this._getBaseUrl()}/unsubscribe?token=${
        ...
        const mailOptions = {
          ...
          subject: subject,
          html: this._buildCarnivalNotificationHtml(
            carnival,
            type,
            headerText,
            carnivalUrl,
        ...
        const enhancedMailOptions = this._addUnsubscribeHeaders(
          mailOptions,
          subscription.unsubscribeToken
        );
  ...
  async sendCarnivalInfoToAttendees(carnival, attendeeClubs, senderName, customMessage = '') {
    try {
      ...
      if (!this._canSendEmails()) {
        this._logBlockedEmail(
          'Carnival Attendees Notification',
          'subscribers',
          `attendees carnival: ${carnival.title}`
        );
        return {
          success: false,
          message: 'Email sending is disabled in the current site mode',
      ...
      const promises = attendeeClubs.map(async (club) => {
        ...
        const mailOptions = {
          ...
          html: this._buildCarnivalInfoToAttendeesHtml(
            carnival,
            club,
            senderName,
            customMessage,
      ...
      console.log(
        `Carnival info emails sent to attendees: ${successful} successful, ${failed} failed`
      );
      ...
      return {
        ...
        message: `Successfully sent carnival information to ${successful} attendee clubs${
  ...
  async sendRegistrationApproval(carnival, club, approverName) {
    try {
      ...
      if (!this._canSendEmails()) {
        this._logBlockedEmail(
          'Carnival Registration Approval',
          'subscribers',
          `registration carnival: ${carnival.title}`
        );
        return {
          success: false,
          message: 'Email sending is disabled in the current site mode',
      ...
      const mailOptions = {
        ...
        html: this._buildRegistrationApprovalHtml(
          carnival,
          club,
          approverName,
          recipient.name,
  ...
  async sendRegistrationRejection(carnival, club, rejectorName, rejectionReason) {
    try {
      ...
      if (!this._canSendEmails()) {
        this._logBlockedEmail(
          'Carnival Registration Rejection',
          'subscribers',
          `rejection carnival: ${carnival.title}`
        );
        return {
          success: false,
          message: 'Email sending is disabled in the current site mode',
      ...
      const mailOptions = {
        ...
        html: this._buildRegistrationRejectionHtml(
          carnival,
          club,
          rejectorName,
          rejectionReason,
  ...
  async sendCarnivalClaimNotification(carnival, claimingUser, claimingClub, originalEmail) {
    try {
      ...
      if (!this._canSendEmails()) {
        this._logBlockedEmail(
          'Carnival Claim Notification',
          originalEmail,
          `carnival claimed: ${carnival.title}`
        );
        return {
          success: false,
          message: 'Email sending is disabled in the current site mode',
      ...
      if (claimingUser.email && claimingUser.email.toLowerCase() === originalEmail.toLowerCase()) {
        console.log(
          `Skipping carnival claim notification - claiming user (${claimingUser.email}) is the same as original contact (${originalEmail})`
        );
        return {
          success: false,
      ...
      const mailOptions = {
        ...
        html: this._buildCarnivalClaimNotificationHtml(
          carnival,
          claimerName,
          claimingClub,
          carnivalUrl
            ...
            <div style="${this._getEmailContainerStyles()}">
                ...
                <div style="${this._getEmailContentStyles()}">
                    ...
                    <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
                        ...
                        ${
                          carnival.scheduleDetails
                            ? `<p><strong>📋 Schedule:</strong> ${carnival.scheduleDetails}</p>`
                            : ''
                        }
   ...
   * @returns {string} HTML content
   */
  _buildCarnivalInfoToAttendeesHtml(
    carnival,
    club,
    senderName,
    customMessage,
            ...
            <div style="${this._getEmailContainerStyles()}">
                ...
                <div style="${this._getEmailContentStyles()}">
                    ...
                    <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
                        ...
                        ${
                          carnival.scheduleDetails
                            ? `<p><strong>📋 Schedule:</strong></p><div style="background: #f8f9fa; padding: 10px; border-radius: 4px; white-space: pre-line;">${carnival.scheduleDetails}</div>`
                            : ''
                        }
                        ...
                        }">${carnival.organiserContactEmail}</a></p>
                        <p><strong>📱 Phone:</strong> <a href="tel:${
                          carnival.organiserContactPhone
                        }">${carnival.organiserContactPhone}</a></p>
                        ...
                        ${
                          carnival.feesDescription
                            ? `<p><strong>💰 Fees:</strong> ${carnival.feesDescription}</p>`
                            : ''
                        }
                ...
                ${this._getEmailFooter(
                  'This email was sent by the carnival organiser to all attending clubs.'
                )}
            </div>
        `;
  }
   ...
   * @returns {string} HTML content
   */
  _buildRegistrationApprovalHtml(
    carnival,
    club,
    approverName,
    contactFirstName,
    ...
    return `
            <div style="${this._getEmailContainerStyles()}">
                ...
                <div style="${this._getEmailContentStyles()}">
                    ...
                    <p>Excellent news! <strong>${approverName}</strong> from the hosting club has approved <strong>${
      club.clubName
    }'s</strong> registration for:</p>
                    ...
                    <p style="font-size: 14px; color: #666;">
                        ...
                        <strong>Email:</strong> <a href="mailto:${
                          carnival.organiserContactEmail
                        }">${carnival.organiserContactEmail}</a><br>
                        <strong>Phone:</strong> <a href="tel:${carnival.organiserContactPhone}">${
      carnival.organiserContactPhone
    }</a>
                    </p>
   ...
   * @returns {string} HTML content
   */
  _buildRegistrationRejectionHtml(
    carnival,
    club,
    rejectorName,
    rejectionReason,
            ...
            <div style="${this._getEmailContainerStyles()}">
                ...
                <div style="${this._getEmailContentStyles()}">
                    ...
                    <p>Thank you for <strong>${
                      club.clubName
                    }'s</strong> interest in attending <strong>${
      carnival.title
    }</strong>. Unfortunately, <strong>${rejectorName}</strong> from the hosting club was unable to approve your registration at this time.</p>
            ...
            <div style="${this._getEmailContainerStyles()}">
                ...
                <div style="${this._getEmailContentStyles()}">
                    ...
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0;">
                        ...
                        <p style="margin: 5px 0 0 0;"><strong>🏛️ Club:</strong> ${
                          claimingClub.clubName
                        }</p>
                        ${
                          claimingClub.location
                            ? `<p style="margin: 5px 0 0 0;"><strong>📍 Club Location:</strong> ${claimingClub.location}</p>`
                            : ''
                        }
                    </div>
                    ...
                    <p style="font-size: 14px; color: #666; text-align: center;">
                        Thank you for organising masters rugby league carnivals! 🏉
                    </p>
                </div>
                
                ${this._getEmailFooter(
                  `Questions? Reply to this email or visit <a href="${this._getBaseUrl()}" style="color: #ccc;">oldmanfooty.au</a>`
                )}
            </div>
        `;
  }
