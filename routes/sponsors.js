...
router.post('/', ensureAuthenticated, ensureAdmin, sponsorUpload, handleUploadError, [
    ...
    body('facebookUrl').optional().isLength({ max: 200 }).withMessage('Facebook URL must be 200 characters or less'),
    body('instagramUrl').optional().isLength({ max: 200 }).withMessage('Instagram URL must be 200 characters or less'),
    body('twitterUrl').optional().isLength({ max: 200 }).withMessage('Twitter URL must be 200 characters or less'),
    body('linkedinUrl').optional().isLength({ max: 200 }).withMessage('LinkedIn URL must be 200 characters or less'),
